  ...
  if (isLoading) {
    return (
      <div className="bg-card border border-border rounded-lg p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Active Conversations</h3>
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto">
        {conversations?.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">New conversations will appear here</p>
          </div>
        ) : (
          conversations?.map((conversation) => (
            <div key={conversation?.id} className="flex items-center space-x-3 p-3 hover:bg-muted/50 rounded-md micro-animation cursor-pointer">
              <div className="relative">
                ...
                />
                <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-card ${getStatusColor(conversation?.status)}`}></div>
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-foreground truncate">{conversation?.name}</p>
                  <span className="text-xs text-muted-foreground">{formatLastSeen(conversation?.lastSeen)}</span>
                </div>
